...
def plot_multiple_hawkins_functions(n, iterations):
    ...
    for i in range(iterations):
        ...
        plt.plot(x_values_h, y_values_h, marker='o', markersize=1, linewidth=0.25, color=colors[i % len(colors)])

    x_avg = list(range(1, len(average_totients) + 1))
    plt.plot(x_avg, average_totients, marker='o', markersize=5, color='black', linewidth=2, label='Average $Φ_H(x)$')
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'Відображення графіків $Φ_H(x)$ та їх апроксимація для {iterations} випадкових наборів')
    plt.grid(True)
    plt.xlim(0, n // 2)
    plt.ylim(0, n)
    plt.show()
